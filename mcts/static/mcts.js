...
var MCTS = {
  ...
  focus: function() {
    return window.wrappedJSObject.AppWindowManager.getActiveApp().iframe;
  },
  ...
  test: function(input) {
    ...
    for (let manifestURL in installedApps) {
      ...
      if (!entryPoints) {
        ...
        if(appName.startsWith("Ce")) {
          app.launch()
          return GaiaApps.sendLocateResponse(marionetteScriptFinished, app, appName, launchPath);
        }
      }
    }
    ...
    for (let manifestURL in installedApps) {
      ...
      if (entryPoints) {
        for (let ep in entryPoints) {
          let ecurrentEntryPoint = entryPoints[ep];
