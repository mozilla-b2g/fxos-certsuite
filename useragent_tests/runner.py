    ...
    for root, dirs, files in os.walk(start_dir, followlinks=True):
        ...
        for file in files:
            ...
            if relpath.endswith(".py"):
                relpath = relpath[:-3]
            name = "useragent_tests.%s" % relpath.replace(os.path.sep, ".")
            module = None
            try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Runner for user agent validation tests.")
    ...
    logger = commandline.setup_logging(
        "useragent", vars(args), {"raw": sys.stdout})
    ...
    tests = test_loader.loadTestsFromNames(
        map(lambda t: "useragent_tests.%s" % t, args.include or [g for g, _ in testgen]), None)
